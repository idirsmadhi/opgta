...
def vidframe2frame_tensors():
  ...
  imgs_med_model = transform_img(img, from_intr=eon_intrinsics, to_intr=medmodel_intrinsics, yuv=True, output_size=(512, 256))
  f2t = frame_to_tensorframe(np.array(imgs_med_model)).astype(np.float32) / 128.0 - 1.0

  # Convertir le tableau Numpy en format d'entrée attendu par ONNX
  f2t_onnx = f2t.astype(np.float32)

  # Effectuer l'inférence avec ONNX Runtime
  inputs = {supercombo.get_inputs()[0].name: f2t_onnx}
  outputs = supercombo.run(None, inputs)
